...
def handle_join():
    """Handle join team commands via API"""
    try:
        team_code = request.args.get('teamcode')
        server = request.args.get('server', '')
        
        if not team_code:
            return jsonify({'status': 'error', 'message': 'Missing teamcode'}), 400
        ...
        return jsonify({
            'status': 'success', 
            'message': f'Join command queued for team: {team_code}'
        }), 200
...
def handle_leave():
    """Handle leave team commands via API"""
    try:
        add_api_command({
            'action': 'leave_team'
        })
...
async def MaiiiinE():
    ...
    Uid , Pw = '4290933539','BY_GRIZLY-OI4QWL6XS-GRIZLY'
    ...
    print(f"   • /emote?teamcode=CODE&emote_id=ID&uids=UID1,UID2")
    print(f"   • /join?teamcode=CODE")
    print(f"   • /leave")
    print(f"   • /status")
...
if __name__ == '__main__':
    ...
    # Run the main bot in background
